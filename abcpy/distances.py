...
class PenLogReg(Divergence):
    ...
    def distance(self, d1, d2):
        ...
        ### Setting reg_inv to 1 (anything other than np.inf) to make this unpenalized logistic regression ###
        ## C = 1 #1e5
        ## l1_ratio_choose makes it l1(1)/l2(0)/elasticnet(0<value_chosen<1)-penalized
        ## This does not take C as those are optimized via cross-validation
        ## max_iter defines how many steps of optimization algorithm is run
        Cs_array = [.01, .05, .075, .1, .5, 1, 2, 5, 10]
        log_reg_model = linear_model.LogisticRegressionCV(Cs=Cs_array, penalty='l1',
                                                          cv= self.n_folds, max_iter=100, solver='liblinear',
                                                        random_state=self.rng.randint(0, np.iinfo(np.uint32).max))
        log_reg_model.fit(training_set_features, training_set_labels)
        #print(1/log_reg_model.Cs_)
        #print(log_reg_model.l1_ratios_)
        #print(log_reg_model.scores_)
        score = np.mean(log_reg_model.scores_[1.0], axis=0)
        print(score)
        distance = 2.0 * (np.mean(log_reg_model.scores_[1.0], axis=0).flatten().max() - 0.5)
...
class LogReg(Divergence):
    ...
    def distance(self, d1, d2):
        ...
        ### Setting reg_inv to np.inf to make this unpenalized logistic regression ###
        reg_inv = np.inf #1e5
        ## l1_ratio = 1 makes it l1-penalized (but as we choose C=np.inf, it becomes unpenalized)
        log_reg_model = linear_model.LogisticRegression(C=reg_inv, l1_ratio= 1, max_iter=100, solver='saga',
                                                        random_state=self.rng.randint(0, np.iinfo(np.uint32).max))
        log_reg_model.fit(training_set_features, training_set_labels)
